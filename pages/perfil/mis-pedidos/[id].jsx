...
const ShowOrder = () => {
  ...
  const { orderDetail, shippingDetail, payments } = useSelector(
    (state) => state.orders
  );
  ...
  const subtotal = helpers.priceFormat(
    orderDetail?.subtotalCurrency || orderDetail?.subtotal,
    orderDetail?.currency?.currency || "MXN"
  );
  const total = helpers.priceFormat(
    orderDetail?.totalCurrency || orderDetail?.total,
    orderDetail?.currency?.currency || "MXN"
  );
  const shippment = helpers.priceFormat(
    orderDetail?.shippmentInCurrency || orderDetail?.shippment,
    orderDetail?.currency?.currency || "MXN"
  );
  ...
  const OrderStatus = ({ status }) => {
    return (
      <div className="ml-2 md:text-sm text-[10px] flex items-center">
        {status === 0 ? (
          <span className="bg-red-500 md:px-10 px-3 text-white rounded-lg">
            Pedido cancelado
          </span>
        ) : status === 1 ? (
          <span className="bg-amber-500 md:px-10 px-3 text-white rounded-lg">
            Pendiente de aprobación
          </span>
        ) : status === 2 ? (
          <span className="bg-blue-500 md:px-10 px-3 text-white rounded-lg">
            Aprobada - Pendiente de envío
          </span>
        ) : (
          <span className="bg-green-500 md:px-10 px-3 text-white rounded-lg">
            Pedido enviado
          </span>
        )}
      </div>
  ...
  return (
    <Layout>
      <BannerImage title="Detalle de la orden" banner="bg-banner7" />
              ...
              <div className="text-sm md:text-base mb-10 mt-3 flex items-center">
                <div className="w-6/12">
                  ...
                  <p className="text-gray-600">
                    {shipping?.shipment_id?.name || "No disponible"}
                  </p>
                </div>
                <div className="w-6/12">
                  <p className="font-bold">No. Guía:</p>
                  <p className="text-gray-600">
                    {shipping?.no_guide || "No disponible"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <tbody>
                {products_list?.map((product) => (
                  ...
                  >
                    ...
                    >
                      <Image
                        src={product.product?.multimedia[0]?.path}
                        alt=""
                      ...
                      <div className="pl-3">
                        ...
                        <div className="font-normal text-gray-500 truncate max-w-[200px]">
                    ...
                    <td className="py-4 px-6">
                      {product.quantity > 1
                        ? `${product.quantity} pzas`
                        : `${product.quantity} pza`}
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      <span className="font-semibold text-gray-600">
                        {helpers.priceFormat(product.subtotalInCurrency)}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      <span className="font-semibold text-gray-600">
                        {helpers.priceFormat(product.totalInCurrency)}
                      </span>
                    </td>
                    {product.product.product_type === "2" &&
                    product.canvas_multimedia.length &&
                    !product.canvasStatus ? (
                      <td>
                        <span
                          className="bg-white w-6/12 flex items-center shadow-lg rounded-md hover:bg-gray-100 cursor-pointer"
                          onClick={() =>
                            handleOpenUploadImages(product, orderDetail._id)
                          }
                        >
                          <button className="bg-green-500 py-1 px-2 text-white rounded-l-md mr-1 text-xs h-full">
                            Subir imagenes
                          </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="md:w-10/12 px-6 w-full  shadow-md">
              ...
              <span className="flex justify-between border-b-2 border-gray-2000 py-2">
                ...
                <p>{`${
                  orderDetail.type == 1 ? "Tarjeta" : " Transferencia"
                }`}</p>
              </span>
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async (ctx) => {
    const isValid = await store.dispatch(
      startGetOrder(ctx.query.id, ctx.req.cookies.token)
    );
