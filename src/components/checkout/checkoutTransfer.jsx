...
export const CheckoutTransfer = ({
  ...
  const handleClickBankAccount = (value) => {
    const bank_account_selected = banksAccounts.find(
      (bank) => bank._id === value
    );
  ...
  const handleFinaliceTransfer = async (e) => {
    ...
    if (banksAccounts.length > 1)
      await dispatch(
        startfinaliceTransferCheckout(
          bankAccountSelected?._id,
          token,
          currency,
          cart
        )
    ...
    <form className="w-full min-h-20 py-3">
      <div className="">
        <FormControl fullWidth size="small">
          <InputLabel id="demo-simple-select-label" sx={{ fontSize: 23 }}>
      ...
      <div className="my-5 min-h-10 border-[1px] border-solid border-[#888]">
        {Object.keys(bankAccountSelected).length > 1 ||
        banksAccounts.length === 1 ? (
          <div className="px-5 py-2 relative">
            ...
            <div className="block w-[100px] h-[100px] mx-auto my-1">
              <Image
                ...
                src={
                  banksAccounts.length > 1
                    ? bankAccountSelected?.bank?.image
                    : banksAccounts[0]?.bank?.image
                }
                width={100}
                height={100}
                alt="backAccount"
            ...
            <div className="flex flex-col justify-between items-center font-Poppins text-base">
              ...
              <span className="text-base leading-7">
                {banksAccounts.length > 1
                  ? bankAccountSelected?.interbank
                  : banksAccounts[0]?.interbank}
              </span>
            </div>
            <div className="flex flex-col justify-between items-center font-Poppins text-base">
              ...
              <span className="text-base leading-7">
                {banksAccounts.length > 1
                  ...
                  : banksAccounts[0]?.account_number}
              </span>
            </div>
            <div className="flex flex-col justify-between items-center font-Poppins text-base">
              <p className="text-[#888] leading-7">
              ...
              <span className="text-base leading-7">
                {banksAccounts.length > 1
                  ? bankAccountSelected?.beneficiary
                  : banksAccounts[0]?.beneficiary}
              </span>
            </div>
          </div>
        ) : (
          <div className="my-10 min-h-10 flex justify-center items-center">
            <p className="text-[#888] leading-6 font-Poppins text-xs">
              <span>Selecciona una cuenta bancaria</span>
            </p>
          </div>
        )}
        <button
          ...
          disabled={
            Object.keys(bankAccountSelected).length > 0 ||
            (banksAccounts.length <= 1 &&
              Object.keys(banksAccounts[0]).length > 0)
              ? false
              : true
          }
        >
